...
def parse_date(value):
    ...
    if m:
        ...
        month = LVar.DUTCH_MONTHS.get(month_name.lower())
        if month:
...
def insert_data():
    ...
    cur.execute("SELECT url FROM album_table")
    existing_album_urls = {row[0] for row in cur.fetchall()}

    cur.execute("SELECT url FROM bijdrage_table")
    existing_bijd_urls = {row[0] for row in cur.fetchall()}

    for album in albums:
        if album['url'] in existing_album_urls:
            continue
        sec = album['sections']
        album_uuid = url_to_uuid[album['url']]
        ...
        for loc in get_section(sec, 'locationCreated'):
            cur.execute(
                "INSERT INTO location_album_table (locationId, albumId, raw_location) VALUES (%s, %s, %s)",
    ...
    for bijd in bijdrages:
        if bijd['url'] in existing_bijd_urls:
            continue
        sec = bijd['sections']
        ...
        for loc in get_section(sec, 'locationCreated'):
            cur.execute(
                "INSERT INTO location_bijdrage_table (locationId, bijdrageId, raw_location) VALUES (%s, %s, %s)",
